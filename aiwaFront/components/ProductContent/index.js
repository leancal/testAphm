...
export default function ProductContent({ prod, features, includes, featuredFeatures }) {
  ...
  const coverImage = prod.img_url && prod.img_url.length > 0 ? prod.img_url[0] : '/path/to/default-image.jpg';
  ...
  return (
    <>
      <section className='top-wrapper' fullscreen={fullscreen ? "true" : "false"}>
        ...
        {mode == 0 && (
          <div className={`default${prod.altBackground ? ' bg-alt' : ''}`} linea={prod.line}>
            ...
            <div className='cover-wrapper'>
              <Image className='cover' src={coverImage} fill alt='product-cover' priority />
            </div>
          </div>
        )}
        {/* {mode == 1 && (
        ...
        {mode == 2 && <ProductMorePhotos arr={prod.img_url} fs={fullscreen} setFs={setFullscreen} sku={prod.sku} shortDesc={prod.shortDesc} />}

        {/* {mode == 3 && <ProductVideos videos={prod.videos} />} */}
      </section>
      <ProductBanners banners={prod.banners} line={prod.line} />
      <ProductFeatures prod={prod} />
      <ProductSpecsTable prod={prod} features={features} includes={includes} />

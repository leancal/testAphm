...
export default function SearchContainer() {
  ...
  // Función para obtener productos de las APIs
  async function fetchProducts() {
    const audioRes = await fetch('/api/audio');
    const cooksRes = await fetch('/api/cooks');
    const headsetsRes = await fetch('/api/headsets');
    const tabletsRes = await fetch('/api/tablets');
    const notebooksRes = await fetch('/api/notebooks');

    const audioData = await audioRes.json();
    const cooksData = await cooksRes.json();
    const headsetsData = await headsetsRes.json();
    const tabletsData = await tabletsRes.json();
    const notebooksData = await notebooksRes.json();

    const allProducts = [...audioData, ...cooksData, ...headsetsData, ...tabletsData, ...notebooksData];
  ...
  return (
    <main className="search">
      ...
      <div className="products">
        ...
        {finalProducts.map((e, i) => <ProductCard key={i} sku={e.sku} products={finalProducts} showName showSku showTags showMenu showViewing />)}
